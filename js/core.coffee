...
adabruMarkup =
  ...
  parseDocument: (document, startNT) ->
    # Parse the input (multi-pass)
    startNT = startNT ? 'Document'

    _parseDocument = (startNT, document) =>
      ast = @parser.match startNT, document

      @visit(
        ast
        (a) -> a.children? and a.children.some((n) -> n.type=='Nextpass')
        (a) ->
          toparse = a.children.map((c)->c.children[0]).join('')
